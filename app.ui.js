...
// TEMAS_CULTO y ESTUDIOS_CALENDARIO viven en app.js
...
function renderEventosPV(){
  ...
  // Banner destacado con el siguiente evento
  let bannerHtml='';
  if(items.length){
    const next=items[0];
    ...
    bannerHtml=`<div class="panel panel-soft-teal" style="margin-bottom:12px;display:flex;align-items:center;gap:12px;">
      ...
      <button type="button" class="btn bteal" style="font-size:11px;padding:7px 10px;white-space:nowrap;" onclick="showPvTab('eventos')">Ver detalles</button>
    </div>`;
  }

  // ── Renderizar próximos en orden ──
  ...
  el.innerHTML=bannerHtml+(html||'<div style="text-align:center;padding:16px;color:#9ca3af;font-size:13px;">No hay eventos próximos.</div>');

  // ── Renderizar eventos pasados (cultos, estudios, eventos) ──
